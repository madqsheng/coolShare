...
1. 对什么分组
   ...
   **输入数据进行分组，并且是只对某一个维度分组，其他维度不变**。前面提到**X维卷积`nn.ConvXd`要在卷积维度**的基础上**+2**，以标准的`nn.Conv2d`为例，卷积的是高度和宽度两个维度，扩展的第一个维度是通道，扩展的第二个维度是批量。那么具体来说，**分组卷积是对输入数据扩展的第一维度进行分组，其他维度不变。**
